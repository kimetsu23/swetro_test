...
function calcularPorcentajeSospecha($velocidad, $velocidad_normal_max, $umbral_sospecha) {
    // Calcula el porcentaje de sospecha
    return (($velocidad - $velocidad_normal_max) / $velocidad_normal_max) * 100;
}
...
function mostrarInformacionPersona($conexion, $UserId) {
    ...
    echo '<thead><tr><th>ID</th><th>Userid</th><th>Velocidad Máxima</th><th>Ritmo Promedio</th><th>Frecuencia Cardíaca</th><th>Porcentaje de Sospecha</th><th>Sospechoso</th></tr></thead>';
    echo '<tbody>';
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            // Calculamos el porcentaje de sospecha
            $velocidad_normal_max = 32; // Velocidad máxima normal en km/h
            $umbral_sospecha = 1.2; // Puedes ajustar este valor según sea necesario
            $porcentaje_sospecha = calcularPorcentajeSospecha($velocidad_maxima, $velocidad_normal_max, $umbral_sospecha);

            // Agregar clases de estilo para resaltar según el rango de porcentaje
            $clase_resaltado = '';
            if ($porcentaje_sospecha >= 0 && $porcentaje_sospecha <= 25) {
                $clase_resaltado = 'bg-success text-white'; // Verde para porcentaje bajo
            } elseif ($porcentaje_sospecha > 25 && $porcentaje_sospecha <= 50) {
                $clase_resaltado = 'bg-warning text-dark'; // Amarillo para porcentaje medio
            } elseif ($porcentaje_sospecha > 50 && $porcentaje_sospecha <= 75) {
                $clase_resaltado = 'bg-orange text-white'; // Naranja para porcentaje alto
            } elseif ($porcentaje_sospecha > 75) {
                $clase_resaltado = 'bg-danger text-white'; // Rojo para porcentaje muy alto
            }

            echo '<tr class="' . $clase_resaltado . '">';
            echo '<td>' . $row['id'] . '</td>';
            echo '<td>' . $row['UserId'] . '</td>';
            echo '<td>' . $velocidad_maxima . '</td>';
            echo '<td>' . $ritmo_promedio . '</td>';
            echo '<td>' . $frecuencia_cardiaca . '</td>';
            echo '<td> <div class="progress-bar" role="progressbar" style="width:'  . number_format($porcentaje_sospecha, 2) . ';" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100"> . number_format($porcentaje_sospecha, 2) . ''</div></td>'; // Mostrar el porcentaje con dos decimales
